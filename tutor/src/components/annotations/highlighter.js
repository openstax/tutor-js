...
TextHighlighter.prototype.doHighlight = function (options = {}) {
  let range = dom(this.el).getRange(), wrapper, createdHighlights, normalizedHighlights, timestamp;
...
TextHighlighter.prototype.highlightRange = function (range, wrapper) {
  ...
  let result = refineRangeBoundaries(range), startContainer = result.startContainer, endContainer = result.endContainer, goDeeper = result.goDeeper, done = false, node = startContainer, highlights = [], highlight, wrapperClone, nodeParent;
...
TextHighlighter.prototype.removeHighlights = function (element) {
  const container = element || this.el, highlights = this.getHighlights({ container: container }), self = this;
...
TextHighlighter.prototype.getHighlights = function (params) {
  ...
  let nodeList = params.container.querySelectorAll('[' + DATA_ATTR + ']'), highlights = Array.prototype.slice.call(nodeList);
...
TextHighlighter.prototype.serializeHighlights = function () {
  const highlights = this.getHighlights(), refEl = this.el, hlDescriptors = [];
  ...
  highlights.forEach(function (highlight) {
    let // Hl offset from previous sibling within parent node.
        offset = 0,
...
TextHighlighter.prototype.deserializeHighlights = function (json) {
  let hlDescriptors, highlights = [], self = this;
...
TextHighlighter.prototype.find = function (text, caseSensitive) {
  const wnd = dom(this.el).getWindow(), scrollX = wnd.scrollX, scrollY = wnd.scrollY, caseSens = (typeof caseSensitive === 'undefined' ? true : caseSensitive);
