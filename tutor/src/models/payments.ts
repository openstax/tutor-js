...
export default class Payments extends BaseModel {
    ...
    fetch() {
        ...
        this.pendingTimeout = window.setTimeout(this.onTimeout, this.options.timeoutLength);
    ...
    onChat() { // we're ignoring the error, not sure how to prefill chat
        const chatLink = document.querySelector<HTMLAnchorElement>('.chat.enabled a');
