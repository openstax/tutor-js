...
Factory.define('Student')
  ...
  .payment_due_at(({ now, days_ago = 0 }) => moment(now || '2019-04-20').add(days_ago + 3, 'days'))
...
Factory.define('Course')
  ...
  .term('spring')
  .year(2017)
  .starts_at(({ now, months_ago = 0 }) => moment(now || '2019-04-20').add(months_ago - 1, 'months').toISOString())
  .ends_at(({ now, months_ago = 0 }) => moment(now || '2019-04-20').add(months_ago + 1, 'months').toISOString())
  .is_active(({ now, object }) => !moment(now || '2019-04-20').isAfter(object.ends_at))
